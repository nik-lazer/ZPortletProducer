 ...
 */
public class TestEventDhtmlLayoutPortlet extends GenericPortlet {
	private static final Log log = Log.lookup(TestEventDhtmlLayoutPortlet.class);
...
//		if (log.debugable()) log.debug("Creates from "+path);
	...
	@ProcessEvent(qname="{http://otr.ru/testevent}testEvent")
	public void processTestEvent(EventRequest request, EventResponse response) {

		Event event = request.getEvent();
		TestEvent testEvent = (TestEvent)event.getValue();
		EventQueue<org.zkoss.zk.ui.event.Event> eq = EventQueues.lookup("portletevent", EventQueues.APPLICATION, true);
		eq.publish(new org.zkoss.zk.ui.event.Event("docType", null, testEvent.getValue()));
	}
...
//			TestEvent testEvent = (TestEvent)event.getValue();
//			EventQueue<org.zkoss.zk.ui.event.Event> eq = EventQueues.lookup("portletevent", EventQueues.DESKTOP, true);
//			eq.publish(new org.zkoss.zk.ui.event.Event("docType", null, testEvent.getValue()));
//
